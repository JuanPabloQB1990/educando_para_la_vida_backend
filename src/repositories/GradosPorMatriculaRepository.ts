...
class GradosPorMatriculaRepository {
  ...
  async create(id_estudiante_periodo: string, id_grado_educacion: string, estado = 'en curso') {
    const [result] = await pool.execute('INSERT INTO grados_por_matricula (id_estudiante_periodo, id_grado_educacion, estado) VALUES (?,?,?)', [id_estudiante_periodo, id_grado_educacion, estado]);
    return { id_estudiante_periodo, id_grado_educacion };
  }
